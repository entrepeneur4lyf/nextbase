...
import (
	...
	"github.com/entrepeneur4lyf/nextbase/tools/logger"
	"github.com/entrepeneur4lyf/nextbase/tools/mailer"
...
func (app *BaseApp) initAuxDB() error {
	...
	// (see https://github.com/entrepeneur4lyf/nextbase/issues/5607)
	...
	app.OnModelAfterDeleteSuccess().Bind(&hook.Handler[*ModelEvent]{
		...
		Func: func(e *ModelEvent) error {
			if m, ok := e.Model.(FilesManager); ok && m.BaseFilesPath() != "" {
				...
				// (https://github.com/entrepeneur4lyf/nextbase/discussions/5246#discussioncomment-10128955)
