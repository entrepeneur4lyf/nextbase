...
import (
	...
	"github.com/nextbase/nextbase/cmd"
	"github.com/nextbase/nextbase/core"
	"github.com/nextbase/nextbase/tools/hook"
	"github.com/nextbase/nextbase/tools/list"
	"github.com/nextbase/nextbase/tools/routine"
	"github.com/spf13/cobra"

	_ "github.com/nextbase/nextbase/migrations"
)

var _ core.App = (*nextbase)(nil)

// Version of nextbase
var Version = "(untracked)"

// nextbase defines a nextbase app launcher.
//
// It implements [core.App] via embedding and all of the app interface methods
// could be accessed directly through the instance (eg. nextbase.DataDir()).
type nextbase struct {
	core.App
...
// New creates a new nextbase instance with the default configuration.
...
// Everything will be initialized when [nextbase.Start] is executed.
// If you want to initialize the application before calling [nextbase.Start],
// then you'll have to manually call [nextbase.Bootstrap].
func New() *nextbase {
...
func NewWithConfig(config Config) *nextbase {
	...
	// https://github.com/nextbase/nextbase/issues/6136
...
// system commands!
func (pb *nextbase) Execute() error {
	if !pb.skipBootstrap() {
...
// so we can have all nextbase flags ready for use on initialization.
func (pb *nextbase) eagerParseFlags(config *Config) error {
	pb.RootCmd.PersistentFlags().StringVar(
		&pb.dataDirFlag,
		"dir",
		config.DefaultDataDir,
		"the nextbase data directory",
	)
...
//
// https://github.com/nextbase/nextbase/issues/404
// https://github.com/nextbase/nextbase/discussions/1267
func (pb *nextbase) skipBootstrap() bool {
	flags := []string{
		"-h",
		"--help",
		"-v",
