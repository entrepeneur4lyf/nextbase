...
// nextbase executable with the latest GitHub release.
//
// Example usage:
//
...
import (
	...
	"github.com/nextbase/nextbase/tools/archive"
	"github.com/nextbase/nextbase/tools/osutils"
...
type Config struct {
	// Owner specifies the account owner of the repository (default to "nextbase").
	Owner string

	// Repo specifies the name of the repository (default to "nextbase").
	Repo string

	// ArchiveExecutable specifies the name of the executable file in the release archive
	// (default to "nextbase"; an additional ".exe" check is also performed as a fallback).
...
func Register(app core.App, rootCmd *cobra.Command, config Config) error {
	...
	if p.config.Owner == "" {
		p.config.Owner = "nextbase"
	}

	if p.config.Repo == "" {
		p.config.Repo = "nextbase"
	}

	if p.config.ArchiveExecutable == "" {
		p.config.ArchiveExecutable = "nextbase"
	}
