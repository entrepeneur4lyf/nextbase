...
import (
	...
	"github.com/nextbase/nextbase/plugins/jsvm"
	"github.com/nextbase/nextbase/tools/list"
	"github.com/nextbase/tygoja"
)
...
// nextbase.nextbase without the on* hook methods
type nextbase = excludeHooks<ORIGINAL_nextbase>

/**
 * ` + "`$app`" + ` is the current running nextbase instance that is globally
 ...
 * For examples and guides please check the [Database guide](https://nextbase.io/docs/js-database).
 *
 * @group nextbase
 */
 ...
 * with the nextbase filesystem abstraction.
 *
 * @group nextbase
 */
...
/**
 * ` + "`" + `$apis` + "`" + ` defines commonly used nextbase api helpers and middlewares.
 *
 * @group nextbase
 */
...
func main() {
	...
	gen := tygoja.New(tygoja.Config{
		Packages: map[string][]string{
			...
			"github.com/nextbase/nextbase/tools/security":   {"*"},
			"github.com/nextbase/nextbase/tools/filesystem": {"*"},
			"github.com/nextbase/nextbase/tools/template":   {"*"},
			"github.com/nextbase/nextbase/mails":            {"*"},
			"github.com/nextbase/nextbase/apis":             {"*"},
			"github.com/nextbase/nextbase/core":             {"*"},
			"github.com/nextbase/nextbase/forms":            {"*"},
			"github.com/nextbase/nextbase":                  {"*"},
			"path/filepath":                                 {"*"},
			"os":                                            {"*"},
			"os/exec":                                       {"Command"},
		},
	...
	result = strings.ReplaceAll(result, "nextbase.nextbase", "nextbase")
	result = strings.ReplaceAll(result, "ORIGINAL_CORE_APP", "core.App")
	result = strings.ReplaceAll(result, "ORIGINAL_nextbase", "nextbase.nextbase")
...
func hooksDeclarations() string {
	...
	for i := 0; i < totalMethods; i++ {
		...
		result.WriteString("/** @group nextbase */")
